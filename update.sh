...
if [[ ${1} == "checkdigests" ]]; then
    ...
    version=$(curl -u "${GITHUB_ACTOR}:${GITHUB_TOKEN}" -fsSL "https://api.github.com/repos/petio-team/petio/releases/latest" | jq -r '.tag_name' | sed s/v//g)
    [[ -z ${version} ]] && exit 1
    old_version=$(jq -r '.version' < VERSION.json)
    changelog=$(jq -r '.changelog' < VERSION.json)
    [[ "${old_version}" != "${version}" ]] && changelog="https://github.com/petio-team/petio/compare/${old_version}...${version}"
